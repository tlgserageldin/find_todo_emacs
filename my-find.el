...
(defconst my-excluded-extensions
  (mapcar #'string-trim (split-string "o,a,png,jpg,jpeg,gif,pdf,zip,tar,gz,7z,bin,exe" "," t)))
...
(defconst my-search-root
  (file-name-directory (directory-file-name my-script-dir))
...
(defun my-search-substring (needle &optional)
...
Results are written to a *grep* buffer with file:line:col so `next-error` / RET works."
  (let* (excluded (my-excluded-extensions))
    (files (directory-files-recursively my-search-root ".*" nil 'follow-symlinks))
